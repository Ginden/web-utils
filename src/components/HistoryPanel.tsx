    ...
    <div style={{ marginTop: '20px' }}>
      ...
      {history.length === 0 ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {history.map((entry) => (
            <li key={entry.timestamp} style={{ marginBottom: '10px', border: '1px solid #ccc', padding: '10px', borderRadius: '5px', display: 'flex', alignItems: 'center' }}>
              ...
              {entry.summary && <div style={{ fontSize: '0.9em', color: '#666', marginTop: '5px', flexGrow: 1 }}>{entry.summary}</div>}
